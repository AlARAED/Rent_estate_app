                                    ...
                                    @foreach($countries as  $key => $country)
                                        <tr>
                                            ...
                                            <td>
                                                @if($country->status == 1  )
                                                    <span
                                                        class="btn btn-success btn-xs">{{ \App\Models\Country::statusList($country->status)}}</span>

                                                @elseif( $country->status == 0)
                                                    <span
                                                        class="btn btn-danger btn-xs">{{ \App\Models\Country::statusList($country->status)}}</span>
                                                @else
                                                    <span
                                                        class="btn btn-primary btn-xs">{!! \App\Models\Country::statusList($country->status) !!}</span>
                                                @endif
                                            </td>
                                            <td>
    ...
    <!--  Modal EditCountry -->
        <div wire:ignore.self class="modal fade " id="EditCountry" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-lg " role="document">
                <div class="modal-content text-center">
                    <div class="modal-header text-center">
                        <h5 class="modal-title text-center" id="exampleModalLongTitle">{{ __('Edit Country') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true close-btn"></span>
                        </button>
                    </div>
